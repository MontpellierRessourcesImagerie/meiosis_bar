...
call("ij.Prefs.set", "global.barPath","/plugins/ActionBar/Meiosis/Meiosis_bar207.ijm");
...
call("ij.Prefs.set", "global.version","v2.06.2");
...
for(i=1; i<	6; i++) {
	tempText="global.annel"+i;
	call("ij.Prefs.set", tempText, true);
	tempText="global.singleChannelTag"+i;
	call("ij.Prefs.set", tempText, "");
}	
defaultStages=newArray("prelepto","earlylepto", "lepto", "leptozygo","earlyzygo","zygo","pachy","diplo");
...
function getFolders(string) {
	//if (File.separator=="\\" && indexOf(string, File.separator)==-1) string=replace(string, "/",File.separator);
	call("ij.Prefs.set", "global.processedFolder",string);
	call("ij.Prefs.set", "global.inputFolder",File.getParent(call("ij.Prefs.get", "global.processedFolder",""))+File.separator);
			...
			for (i = start; i < imagesList.length; i++) {
				if(File.isDirectory(inputFolder+File.separator+imagesList[i])==false){
					for (j = 0; j < stageArray.length; j++){
						if(startsWith(toLowerCase(imagesList[i]), toLowerCase(stages[j]))&& endsWith(toLowerCase(imagesList[i]), ".tif")) return(i);	
					}
				}
			}
			return(-1);
				...
				if (parseInt(call("ij.Prefs.get", maximaTempText, -1))>-1) {// some type maximum initially identified
					...
					if (Roi.size>0) {// is a type particle
						...
						if (call("ij.Prefs.get", "global.showIndividualFoci",false)) {
							...
							if (type=="total")roiManager("rename", "focus"+letter+"_"+nType+" (maximum)");
							else roiManager("rename", type+"_focus"+letter+"_"+nType+" (maximum)");
						}
						...
						roiManager("select", particleRoi);
						if (type=="total") tempRoiName="focus"+letter+"_"+nType+" (particle)";
						else tempRoiName=type+"_focus"+letter+"_"+nType+" (particle)";
						roiManager("rename", tempRoiName);
						...
						if (parseInt(call("ij.Prefs.get", maximaTempText, -1))>-1){// some antitype maxima initially identified
							...
							if (Roi.size>0) {// the particle correspond to an antitype maximum
								...
								if(call("ij.Prefs.get", "global.showIndividualFoci", false)) {
									...
									roiManager("rename", antiType+"_focus"+letter+"_"+nAntiType+" (maximum)");
								}
								else roiManager("deselect");		
								roiManager("select", particleRoi);
								RoiManager.setGroup(getRoiGroupFromBinary(antiType, letter, "", "particle"));
								roiManager("rename", antiType+"_focus"+letter+"_"+nAntiType+" (particle)");
...
Dialog.addMessage("Display options");
...
Dialog.addMessage("Reference ROI identification parameters");
Dialog.addChoice("Reference mode", ROIModes, call("ij.Prefs.get", "global.ROImode",""));
Dialog.addNumber("Nucleus channel (for nucleus threshold mode):", call("ij.Prefs.get", "global.nucleusChannel",-1));
Dialog.addToSameRow(); 
Dialog.addChoice("Nucleus Threshold", nucleusThresholds, call("ij.Prefs.get", "global.nucleusThreshold",""));
...
Dialog.addMessage("Foci Colocalisation Parameters");
...
Dialog.addMessage("Intensity measurements parameters");
...
if (call("ij.Prefs.get", "global.tool", "")=="") showMessage("error", "Click on start checking results first");
	...
	counter=parseInt(call("ij.Prefs.get", "global.lastImageFileProcessed",-1));
	if (call("ij.Prefs.get", "global.processedFolder","")=="") {
		getFolders(getDirectory("Choose the folder containing the cropped and stage-sorted images that were analysed"));
	}
...
while (counter>-1);
if (analysedImages==0) {showMessage("There are no meiosis stage image to analyse!");}	
else showMessage("Analysis done", "All advanced parameters tested on input images");
if (!call("ij.Prefs.get", "global.debugMode", false)) setBatchMode(false);
</macro>
</line>
/////////////////////////////////
